        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div className="space-y-6">
            ...
            {socialLinks.length > 0 && (
              <div className="flex gap-4">
                {socialLinks.map((s) => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="text-warm-400 transition-colors hover:text-blue-400" aria-label={s.label}>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-sm text-warm-300 transition-colors hover:text-blue-400">{item.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {SERVICES.map((service) => (
                <li key={service.slug}>
                  <Link href={`/services/${service.slug}`} className="text-sm text-warm-300 transition-colors hover:text-blue-400">{service.title}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                ...
                <a href={`tel:${settings.phone}`} className="text-sm text-warm-300 transition-colors hover:text-blue-400">{settings.phone}</a>
              </li>
              <li className="flex items-start gap-3">
                <Mail size={16} className="mt-0.5 shrink-0 text-blue-400" />
                <a href={`mailto:${settings.supportEmail}`} className="text-sm text-warm-300 transition-colors hover:text-blue-400">{settings.supportEmail}</a>
              </li>
              <li className="flex items-start gap-3">
